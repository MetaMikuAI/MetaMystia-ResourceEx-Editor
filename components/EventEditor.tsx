...
import type { EventNode, MissionNode, Character } from '@/types/resource';
...
const TRIGGER_TYPES = [
	...
	{
		value: 'BuyCount',
		label: '【未实现】【未实现】购买了目标物品X个数量时(BuyCount)', // 第二个未实现是游戏本身未实现
	},
	{
	...
	{ value: 'OnChallengeEnd', label: '【未实现】挑战结束前(OnChallengeEnd)' },
	{
		...
		label: '【未实现】【已弃用】挑战阶段切换时(Obsolete_OnChallengePhaseChange)', // 第二个已弃用是游戏本身已弃用
	},
	{
				...
				<div className="flex flex-col gap-2">
					...
					<div className="flex flex-col gap-2 rounded-lg border border-black/10 bg-black/5 p-4 dark:border-white/10 dark:bg-white/5">
						<div className="flex flex-col gap-1">
							...
							<select
								value={eventNode.trigger?.triggerType || ''}
								onChange={(e) => {
									...
									if (
										...
										eventNode.trigger?.triggerId
									) {
										newTrigger.triggerId =
											eventNode.trigger.triggerId;
									}
									onUpdate({ trigger: newTrigger });
								}}
						...
						{eventNode.trigger?.triggerType ===
							...
							<div className="flex flex-col gap-1">
								...
								<select
									value={eventNode.trigger?.triggerId || ''}
									onChange={(e) =>
										onUpdate({
											trigger: {
												...eventNode.trigger!,
