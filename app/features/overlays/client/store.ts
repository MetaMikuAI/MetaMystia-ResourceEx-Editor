...
interface IOverlayShortcutDispatchResult {
	handled: boolean;
	matched: boolean;
}
...
function checkOrderedArrayEqual<T>(
	leftItems: ReadonlyArray<T>,
	rightItems: ReadonlyArray<T>
) {
	return (
...
function flushPassiveQueue() {
	...
	if (
		...
		checkOrderedArrayEqual(snapshot.passiveQueue, remainingQueue)
	) {
		return;
	}
...
function coverActivePassiveOverlay() {
	const { passiveActiveId, passiveQueue } = snapshot;
	...
	patchSnapshot({
		...
		passiveQueue: [
			passiveActiveId,
			...passiveQueue.filter((id) => id !== passiveActiveId),
		],
	});
}
...
function dispatchOverlayShortcut(
	event: KeyboardEvent
